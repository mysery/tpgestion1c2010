...
*/
        ...
        private RNode<T> adjustTree(RNode<T> n, RNode<T> nn)
        {
            ...
            while (n.level != treeHeight)
            {
                ...
                RNode<T> parent = getNode(parents.Pop());
         ...
         */
        private void checkConsistency(int nodeId, int expectedLevel, RRectangle expectedMBR)
        {
            // go through the tree, and check that the internal data structures of 
            // the tree are not corrupted.    
            RNode<T> n = getNode(nodeId);


            RRectangle calculatedMBR = calculateMBR(n);
         ...
         */
        private RRectangle calculateMBR(RNode<T> n)
        {
            RRectangle mbr = new RRectangle(n.entries[0].min, n.entries[0].max);
